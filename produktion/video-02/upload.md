# Upload — Video 02

> Automatisch aus der Pipeline erzeugt. Alles unten wird beim Upload
> **von Hand** eingetragen.

## Vor dem Upload zu erledigen

- [ ] **KI-Kennzeichnung setzen** („Altered or synthetic content" /
      „Realistic audio"). Die Stimme ist synthetisch. Formel §7 und §8
      führen das als Compliance-Entscheidung ohne Datenbeleg in beide
      Richtungen.
- [ ] **Thumbnail ersetzen.** Im Paket liegt nur
      `PLATZHALTER_standbild.png` — das ist das Standbild der Videospur,
      **nicht** das Thumbnail.
      Motivvorgabe: Grundmotiv, aufgeschlagenes Buch neben der sitzenden Figur (belegt in A's 245K), Feuer etwas größer und wärmer, Himmel mit dichterem Sternenfeld.
      Thumbnail-Text: `QUIET YOUR MIND` — Versalhöhe ≥ 11,5 % der
      Bildhöhe, Kontrast ≥ 10:1, höchstens 4 Wörter
      (`formel/thumbnail-checkliste.md`).
- [ ] Untertiteldatei hochladen (Sprache: Englisch).
- [ ] Sichtbarkeit/Zeitplan nach `produktion/videos-01-08.md` (5 Tage Abstand).
- [ ] Spendenlink in der Beschreibung ersetzen.

## Titel

```
Stop Thinking For A Moment, and Let God's Wisdom Quiet You Tonight
```

## Beschreibung

```
Stop Thinking For A Moment, and Let God's Wisdom Quiet You Tonight

For the nights when your mind won't stop working. Psalms 90 through 150 and the whole
book of Proverbs, read slowly and without interruption — nothing to follow, nothing
to remember.

Let it play, close your eyes, and let the noise go quiet on its own.

Focused on:
• A mind that keeps circling
• Wisdom instead of worry
• Letting the night be the night
• Falling asleep in God's presence

Read from the World English Bible (British Edition).

Rest well tonight. Subscribing helps you find the next reading.

Support the channel: [Spendenlink]

#biblesleep #proverbs #psalmsforsleep #christiansleep #bibleversesforsleep

Kapitel:
0:00 Opening prayer
1:40 Psalm 90
3:42 Psalm 91
5:37 Psalm 92
7:15 Psalm 93
7:56 Psalm 94
10:39 Psalm 95
11:35 Psalm 96
13:11 Psalm 97
14:36 Psalm 98
15:48 Psalm 99
16:59 Psalm 100
17:39 Psalm 101
18:49 Psalm 102
21:49 Psalm 103
24:14 Psalm 104
28:14 Psalm 105
32:40 Psalm 106
37:58 Psalm 107
42:29 Psalm 108
43:57 Psalm 109
47:25 Psalm 110
48:20 Psalm 111
49:33 Psalm 112
50:46 Psalm 113
51:37 Psalm 114
52:26 Psalm 115
54:18 Psalm 116
56:13 Psalm 117
56:30 Psalm 118
59:38 Psalm 119
1:16:17 Psalm 120
1:16:59 Psalm 121
1:17:44 Psalm 122
1:18:36 Psalm 123
1:19:13 Psalm 124
1:20:00 Psalm 125
1:20:45 Psalm 126
1:21:26 Psalm 127
1:22:09 Psalm 128
1:22:52 Psalm 129
1:23:45 Psalm 130
1:24:33 Psalm 131
1:25:03 Psalm 132
1:27:01 Psalm 133
1:27:30 Psalm 134
1:27:50 Psalm 135
1:30:05 Psalm 136
1:32:42 Psalm 137
1:33:55 Psalm 138
1:35:08 Psalm 139
1:37:45 Psalm 140
1:39:28 Psalm 141
1:40:54 Psalm 142
1:41:55 Psalm 143
1:43:41 Psalm 144
1:45:43 Psalm 145
1:48:05 Psalm 146
1:49:23 Psalm 147
1:51:27 Psalm 148
1:52:52 Psalm 149
1:53:52 Psalm 150
1:54:30 Proverbs 1
1:58:10 Proverbs 2
2:00:17 Proverbs 3
2:03:54 Proverbs 4
2:06:48 Proverbs 5
2:09:11 Proverbs 6
2:13:06 Proverbs 7
2:15:57 Proverbs 8
2:19:50 Proverbs 9
2:21:51 Proverbs 10
2:25:30 Proverbs 11
2:29:21 Proverbs 12
2:32:38 Proverbs 13
2:35:31 Proverbs 14
2:39:27 Proverbs 15
2:43:10 Proverbs 16
2:46:50 Proverbs 17
2:50:14 Proverbs 18
2:52:53 Proverbs 19
2:56:20 Proverbs 20
2:59:44 Proverbs 21
3:03:07 Proverbs 22
3:06:22 Proverbs 23
3:10:03 Proverbs 24
3:13:52 Proverbs 25
3:17:13 Proverbs 26
3:20:19 Proverbs 27
3:23:30 Proverbs 28
3:26:54 Proverbs 29
3:29:58 Proverbs 30
3:34:13 Proverbs 31
```

## Tags

Formel §7: A hat auf **allen 8** Videos 0 Tags, B's drei gemessene Treffer
ebenfalls 0. Sie kosten nichts — erwarte aber nichts von ihnen.

```
proverbs for sleep, psalms for sleep, bible for sleep, christian sleep meditation, wisdom for sleep, overthinking help, scripture for sleep, book of proverbs, bible reading for sleep, sleep with god's word, calm my mind, bedtime bible, world english bible, peaceful scripture, christian bedtime
```

## Kapitelmarken (93)

Formel §7: **optional, nicht Pflicht.** A's drei größte Treffer haben
null Kapitelmarken, B setzt sie durchgehend. Beide Muster gewinnen —
hier für die Nutzbarkeit gesetzt, nicht für die Reichweite.

Sie stehen bereits in der Beschreibung oben.

## Dateien im Paket

- `video-02.mp4` ✓
- `video-02.srt` ✓
- `PLATZHALTER_standbild.png` ✓

## Messwerte dieses Renderlaufs

> **Diese Tabelle ist nicht belegbar (Vermerk 2026-08-26).** Die Werte stammen
> aus den QA-Dateien des Renderlaufs in `produktion/arbeit/` — und das
> Verzeichnis steht in `.gitignore`. Die Quelle existiert nicht mehr. Nach der
> Regel in `produktion/workflow-gates.md` („Prozessbefund — was als gemessen
> gilt") gelten diese Zahlen damit **nicht als gemessen**: sie sind maschinell
> erzeugt, aber im Repository nicht nachprüfbar.
>
> Ab Video 05 schreibt `schritt7_paket.py` die Messwerte als `qa.json` neben
> `upload.md`, mit Commit und Config-Prüfsumme. **Nicht rückwirkend** — für
> V01–V04 ist die Lücke nicht mehr zu schließen.


| Größe | Wert | Vorgabe |
|---|---|---|
| Laufzeit | 3:37:32 (3.62 h) | ≥3.0 h, Ziel 3.4–3.8 h |
| Dateigröße | 2415.3 MB | — |
| Bild | 1920x1080 @ 24 fps | 1920×1080, 24–30 fps (§5) |
| Ton-Versatz | 0.0 s | 0 |
| Tempo | 141.8 WPM | 120–160 WPM (§5b) |
| Sprachanteil (Lücken <1 s zugerechnet) | 95.3 % | ≥97.0 % (§3) |
| längste Pause | 1.38 s | <20.0 s (§3) |
| Abstand Stimme zu Bett | 12.0 dB | 12.0 dB (§5b) |
| Peak | -0.3 dBFS | <-0.3 dBFS |
| Untertitelkacheln | 3128 | — |
| erste Kachel | 1.5 s | Sprache in Sekunde 0–3.0 (§3) |
| CTA | 2 | höchstens 2 (§3) |
| TTS-Zeichen | 162.244 | — |
| Standbild dunkel | 93.9 % der Fläche | dunkles Gesamtbild (§5) |

## Stimme

`MILO SOOTHING VOICE` — Fish Audio `cb6381fb822345bd89c207fb49551d24`, Modell s2.1-pro-free, Tempo 0.88.
Feste Kanalstimme, siehe `produktion/config.md`.

