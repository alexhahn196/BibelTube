...
|---|---|---|---|---|
| 1.1 | **Korpuslänge** | ≥ 3,0 h; Ziel 3,4–3,8 h — **3,0–3,8 h**, wenn das dominante Buch selbst Erzählwerk ist und in voller Länge im Korpus steht | Formel §2: kein Video unter 3 h je über 2.500 Views (n=6), alle 10 Treffer ≥ 3,2 h. Die Ausnahme ab 2026-09-02, siehe unter der Tabelle | Grenzen in [`produktion/config.md`](config.md) (`laufzeit_*`); `schritt1_text.py` (Schätzung) und `schritt5_video.py` (fertige Datei) — **beide brechen ab**, wenn die harte Untergrenze reißt |
...
### Die Entscheidungen vom 02.09.2026

**Alle Schwellen von 1.1 und 1.13 stehen ausschließlich in
[`produktion/config.md`](config.md)** (`laufzeit_min_h`, `laufzeit_ziel_von_h`,
`laufzeit_ziel_von_h_vollwerk`, `laufzeit_ziel_bis_h`, `gate_erzaehlanteil_min`,
`gate_dominanz_min`, `gate_abstand_min`). Diese Tabelle, `erfolgsregeln.md`, der
Skill und beide Prüfskripte lesen von dort. Wer eine Grenze ändern will, ändert
sie an einer einzigen Stelle.

#### 1.13 ist die Strukturfassung — der Prozentwert gatet nicht

Bis zum 02.09. gab es dazu **zwei eingecheckte Fassungen auf zwei Zweigen**. Der
Kanalinhaber hat die Strukturfassung gewählt. 1.13 fragt seither nach der Bauart
des Korpus, nicht nach seinem Prozentwert:
...
Größe widersprachen sich: buchweise hielt V05 mit 81,7 %, kapitelweise fiel es
mit 47,6 % durch. **V03 — das einzige Video des Kanals, das funktioniert hat —
fällt in beiden** (62,3 % / 38,2 %). Die 80 % können also nicht die Größe sein,
an der V03 gegen V02 gewonnen hat; belegt ist die **Struktur**, Evangelium gegen
Spruchsammlung, nicht 80 gegen 79. Kein eigenes Video liegt kapitelweise über
47,6 %. Der Wert wird weiter kapitelweise gemessen und in jedem Prüflauf
**gemeldet** — als Zahl, die man kennen muss, nicht als Grenze, die abbricht.
...
Zielband und Dominanz klemmten sich gegenseitig ein. Bei 143,7 WPM beginnt das
Band 3,4–3,8 h bei **29.315 W**; ein dominantes Buch mit 60 % davon musste
mindestens **17.589 W** haben. Markus (14.261 W) fiel darunter, ganz Genesis
(35.827 W) sprengt das Band allein — beide an der Größe, nicht an der Bauart.

| | vorher | jetzt | warum das ging |
|---|---|---|---|
| Dominanz | ≥ 60 % | **≥ 50 %** | selbstgesetzt, durch keine Messung gedeckt. Der Eigenname trägt Titel und Thumbnail auch bei der Hälfte der Laufzeit — dass er die *Hauptsache* trifft, sichert jetzt `gate_abstand_min`. |
| Untere Bandgrenze | 3,4 h | **3,0 h**, nur bei ganzem Erzählwerk als dominantem Buch | 3,4–3,8 h ist der Median der **Fremd**-Treffer (Formel §2), eine Beobachtung an anderen Kanälen. Die harte Untergrenze **3,0 h** (`laufzeit_min_h`) ist unberührt. |

> **Nebenwirkung, offen ausgesprochen:** die Strukturfassung von 1.13 verlangt
> „Erzählwerk in voller Länge" ohnehin. Jeder Korpus, der 1.13 hält, bekommt
> damit **automatisch** das tiefere Band; die 3,4 h greifen nur noch bei
> Korpora, die schon durchgefallen sind. Der Wert bleibt stehen, weil er die
> Bandgrenze der Formel ist und bei einer Rückkehr zur Prozentfassung wieder
> wirksam würde.
...
— siehe M8. Sie greift **vor** dem Textbau, weil sie den Korpus betrifft und
nicht das Rendering: gerissen wird sie am Reißbrett, nicht in der Pipeline.
...
**Die 80 % gaten seit dem 02.09.2026 nicht mehr — sie sind die Schwelle einer
Ja/Nein-Frage.** M8 trennt Erzählstoff von Spruchsammlung, sagt aber nichts über die
Grenze dazwischen. Als Gate über den Gesamtkorpus war die Zahl von keiner eigenen
Messung berührt; als Antwort auf „ist das dominante Buch überhaupt Erzählwerk" ist
sie belastbar, weil dort der Abstand groß ist und nicht an der Körnung hängt.
`korpus_pruefung.py` und `erzaehlanteil.py` melden den tatsächlichen Prozentsatz des
Gesamtkorpus immer mit — als Zahl, die man kennen muss, nicht als Grenze, die
abbricht. Zum Vergleich: der gestrichene Jesaja-Korpus von V06 liegt kapitelweise bei
**7,3 %**, das gebaute V06 bei **89,0 %**.
